...
class Vetores:
    ...
    def gerar_vetor_desordenado(tamanho, tipo):
        ...
        if tipo == "aleatorio": # Gera um vetor de números aleatórios, limitando entre 0 e 1.000.000
        ...
        elif tipo == "poisson": # Gera um vetor de números aleatórios com distribuição de poisson, limitando entre 0 e 1.000.000
            vetor_desordenado = np.random.poisson(1, tamanho)
            vetor_desordenado = [max(0, min(int(x), 1000000)) for x in vetor_desordenado]
        elif tipo == "exponencial_negativa": # Gera um vetor de números aleatórios com distribuição exponencial negativa, limitando entre 0 e 1.000.000
